...
int main() {
	...
	FILE *fptr;
	int i;
	fptr=fopen("F:\\cprograms\\MeghaGrover_99_C_Program_Repository\\31_ReadWriteMarks.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
	...
	fclose(fptr);
	fptr=fopen("F:\\cprograms\\MeghaGrover_99_C_Program_Repository\\31_ReadWriteMarks.txt","rb");
